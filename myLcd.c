...
const char symbol[2][2] =
{
    {0x01, 0x00},  /* "-" LCD segments a+b+c+d+e+f+k+q */
    {0x21, 0x10}  /* "." */
};

const char equal[1][2] =
{
    {0x13, 0x00}  /* "-" LCD segments a+b+c+d+e+f+k+q */
};
...
void myLCD_showChar( char c, int Position )
{
    ...
    if ( ( Position >= 0 ) && ( Position <= 6 ) )
    {
        ...
        {
            LCD_C_setMemory( LCD_C_BASE, SegmentLoc[Position][0], symbol[c-45][0] );
            LCD_C_setMemory( LCD_C_BASE, SegmentLoc[Position][1], symbol[c-45][0] >> 4 );
            LCD_C_setMemory( LCD_C_BASE, SegmentLoc[Position][2], symbol[c-45][1] );
            LCD_C_setMemory( LCD_C_BASE, SegmentLoc[Position][3], symbol[c-45][1] >> 4 );
        }
        else if ( c >= '=' && c <= '=' )
        {
            LCD_C_setMemory( LCD_C_BASE, SegmentLoc[Position][0], equal[c-61][0] );
            LCD_C_setMemory( LCD_C_BASE, SegmentLoc[Position][1], equal[c-61][0] >> 4 );
            LCD_C_setMemory( LCD_C_BASE, SegmentLoc[Position][2], equal[c-61][1] );
            LCD_C_setMemory( LCD_C_BASE, SegmentLoc[Position][3], equal[c-61][1] >> 4 );
        }
        else
        {
